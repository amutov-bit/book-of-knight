export const STRIPS_CONFIG = Object.freeze({
    activeStripSet: 'normal',
    stripsNormalGames: [
        [
...
function scanNumericArrays(value, maxRef) {
    if (!Array.isArray(value)) return;
    for (let i = 0; i < value.length; i++) {
        const item = value[i];
        if (Array.isArray(item)) {
            scanNumericArrays(item, maxRef);
            continue;
        }
        if (Number.isFinite(item) && item > maxRef.max) {
            maxRef.max = item;
        }
    }
}

export function getMaxStripSymbolIndex() {
    const maxRef = { max: 0 };
    scanNumericArrays(STRIPS_CONFIG.stripsNormalGames, maxRef);
    scanNumericArrays(STRIPS_CONFIG.stripsFreeGames, maxRef);
    scanNumericArrays(STRIPS_CONFIG.stripsHoldAndWin, maxRef);
    return maxRef.max;
}
