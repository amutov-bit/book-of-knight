...
public class ButtonSettings extends Actor {
	
	BookOfKnight game;
	
	TextureAtlas textureAtlas;
	private Array<AtlasRegion> regions = new Array<AtlasRegion>();	
	private Array<AtlasRegion> regionsSlider = new Array<AtlasRegion>();	
...
//	private boolean toggled = false;
	...
	private List<Label>textDisable = new ArrayList<Label>();
	 ...
	 * Constructor
	 */
	public ButtonSettings(BookOfKnight game, int x, int y, TextureAtlas textureAtlas, String buttonName) {
		this.game = game;
...
//		toggled = false;
	...
	public void commitAssets() {
		...
		regionsSlider = textureAtlas.findRegions("slider");
		regionsSlider.add(textureAtlas.findRegion("slider_deact"));
...
//		Gdx.app.debug("Frame = ", "totalFrames = "  + totalFrames);
//		Gdx.app.debug("Frame = ", "toggledFrame = " + toggledFrame);
//		Gdx.app.debug("Frame = ", "hoverFrame = "   + hoverFrame);
...
//		#b9c5d3 185, 197, 211
		text.add(new Label(" ", new LabelStyle(game.fonts.font36px, new Color(185/255f, 197/255f, 211/255f, 255f/255f))));
		
		text.add(new Label(" ", new LabelStyle(game.fonts.font36px, new Color(185/255f, 197/255f, 211/255f, 255f/255f))));
//		text.get(0).setPosition(xX,yY,(Align.center));
//		text.get(0).setBounds(xX,yY,regions.get(0).getRegionWidth(),regions.get(0).getRegionHeight());
		
		
//		text.get(1).setPosition(xX,yY, (Align.center));
//		text.get(1).setBounds(xX,yY,regions.get(0).getRegionWidth(),regions.get(0).getRegionHeight());
		
//		text.get(2).setPosition(xX,yY,(Align.center));
//		text.get(2).setBounds(xX,yY,regions.get(0).getRegionWidth(),regions.get(0).getRegionHeight());
...
//    			regions.get(currentFrame).getRegionHeight());
		...
		offsetX = (regionsSlider.get(currentFrame).getRotatedPackedWidth() - regionsSlider.get(currentFrame).getRegionWidth())  / 2;
		offsetY = (regionsSlider.get(currentFrame).getRotatedPackedHeight() - regionsSlider.get(currentFrame).getRegionHeight())  / 2;
		regionsSlider.get(currentFrame).getTexture().setFilter(TextureFilter.Linear, TextureFilter.Linear);
		batch.draw(regionsSlider.get(currentFrame).getTexture(),
				getX() + 305,
				getY(), 
				(float)regionsSlider.get(currentFrame).getRegionWidth()/2,
				(float)regionsSlider.get(currentFrame).getRegionHeight()/2,
				(float)regionsSlider.get(currentFrame).getRegionWidth(),
				(float)regionsSlider.get(currentFrame).getRegionHeight(),
				1f,
				1f,
				regionsSlider.get(currentFrame).rotate ? -90f : 0f,
						regionsSlider.get(currentFrame).getRegionX(), 
						regionsSlider.get(currentFrame).getRegionY(),  
						regionsSlider.get(currentFrame).getRegionWidth(),
						regionsSlider.get(currentFrame).getRegionHeight(),
						false,
						false);
		
//		Gdx.app.debug("draw", "getX() = " + getX());
		if(disabledFrame == currentFrame){
			text.get(0).setAlignment(Align.left);
			text.get(0).setBounds(getX() + 80,getY(), getWidth(), getHeight());
			text.get(0).draw(batch, 0.1f);
		} else {
			text.get(0).setAlignment(Align.left);
			text.get(0).setBounds(getX() + 80,getY(), getWidth(), getHeight());
			text.get(0).draw(batch, alpha);
		}
    }	
