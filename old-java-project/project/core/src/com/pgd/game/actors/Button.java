...
//	private boolean toggled = false;
	...
	private int autoPlayCnt = 0;
	
	/**
	 * Constructor
	 */
	public Button(BookOfKnight game, int x, int y, TextureAtlas textureAtlas, String buttonName) {
		this.game = game;
...
//			    	Gdx.app.debug("Button", "released");
		...
		text.add(new Label(" ", new LabelStyle(game.fonts.font54px, new Color(255f/255f, 255f/255f, 255f/255f, 255f/255f))));
		text.get(0).setFontScale(1f);
	}
...
//		Gdx.app.debug("Button ", "this.buttonName = " + this.buttonName);
//		Gdx.app.debug("Button ", "currentFrame = " + currentFrame);
		...
		if(this.buttonName == "button_autoX"){
//			if(autoPlayCnt >= 100){
//				text.get(0).setFontScale(0.7f);
//			} else {
				text.get(0).setFontScale(1f);
//			}
			
			text.get(0).setAlignment(Align.center);
			text.get(0).setBounds(getX() + 35, getY() - 6 + 1 + 30, 90, 90);
			text.get(0).draw(batch, alpha);
		}
    }	
    
    public void setText(String str, int cnt){
    	autoPlayCnt = cnt;
